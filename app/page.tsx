...
function initialTrashedSet(): Set<string> {
  const stored =
    typeof window === "undefined" ? new Set<string>() : loadSet(TRASH_KEY)
...
export default function Page() {
  ...
  const [desktopOrder, setDesktopOrder] = useState<string[]>(() =>
    DESKTOP.filter((r) => !initialTrashedSet().has(r.item.id)).map(
      (r) => r.item.id
    )
  )
  ...
  useLayoutEffect(() => {
    const handleResize = () => {
      setRows(Math.floor((window.innerHeight - GRID_PAD * 2) / CELL_SIZE))
      setColumns(Math.floor((window.innerWidth - GRID_PAD * 2) / CELL_SIZE))
    }
    handleResize()
  ...
  const taskbarItems = [...minimizedItems]
    .map((id) => {
      if (id === "__bin__")
        return { id, title: "bin", color: "oklch(0.577 0.245 27.325)" }
      const item = findItem(id)
      return item
        ? { id, title: item.title, color: item.color ?? "var(--brand-light)" }
        : null
    })
